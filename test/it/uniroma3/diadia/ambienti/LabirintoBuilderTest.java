...
public class LabirintoBuilderTest {
	...
	public void testMonolocaleConAttrezzoSingoloDuplicato() {
		...
		assertTrue(size==1);
		List<Attrezzo> attrezziInStanzaIngresso = new ArrayList<>(monolocale.getStanzaCorrente().getAttrezzi());
	...
	public void testBilocale() {
		...
		assertEquals(Collections.singletonList("nord").toString(),bilocale.getStanzaCorrente().getDirezioni().toString());
	...
	public void testPiuDiQuattroAdiacenti() {
		...
		assertTrue(maze.getStanzaCorrente().getDirezioni().size()<=4);
		assertTrue(!maze.getStanzaCorrente().getDirezioni().containsValue(test));
		...
		assertEquals(mappa.keySet().toString(), maze.getStanzaCorrente().getDirezioni().keySet().toString());
		assertEquals(mappa.values().toString(), maze.getStanzaCorrente().getDirezioni().values().toString());
	}
	...
	public void testAggiuntaAttrezziAStanze_Iniziale() {
		...
		assertEquals(attrezzo.toString(), maze.getStanzaCorrente().getAttrezzo(nomeAttrezzo).toString());
	}
	...
	public void testAggiuntaAttrezziAStanze_AppenaAggiunte() {
	    ...
	    labirintoBuilder
	            ...
	            .addAttrezzo(nomeAttrezzo, peso)
	            .getLabirinto();
	    assertTrue(labirintoBuilder.getListaStanze().get(nomeStanza).getAttrezzi().toString().contains(nomeAttrezzo));
	...
	public void testLabirintoConStanzaBloccata_ConPassepartout() {
		...
		stanzaVincente.impostaStanzaAdiacente("sud", stanzaBloccata);
	...
	public void testLabirintoConStanzaBloccata_SenzaPassepartout() {
		...
		stanzaBloccata.impostaStanzaAdiacente("nord", stanzaVincente);
		stanzaBloccata.impostaStanzaAdiacente("sud", stanzaIniziale);
	...
	public void testLabirintoCompletoConTutteLeStanze() {
		...
		Stanza stanzaIniziale = labirintoCompleto.getStanzaCorrente();
		Stanza corridoio = labirintoCompleto.getStanzaCorrente().getStanzaAdiacente("nord");
		Stanza corridoioBloccato = labirintoCompleto.getStanzaCorrente().getStanzaAdiacente("nord").getStanzaAdiacente("nord");
		Stanza stanzaMagica = labirintoCompleto.getStanzaCorrente().getStanzaAdiacente("nord").getStanzaAdiacente("est");
		Stanza stanzaBuia = labirintoCompleto.getStanzaCorrente().getStanzaAdiacente("nord").getStanzaAdiacente("ovest");
		...
		assertTrue(corridoio.getDirezioni().containsKey("ovest"));
		assertTrue(corridoio.getDirezioni().containsKey("nord"));
		assertTrue(corridoio.getDirezioni().containsKey("sud"));
		assertTrue(corridoio.getDirezioni().containsKey("est"));
		...
		assertEquals(mapAdiacenti.toString(),corridoio.getDirezioni().toString());
		Attrezzo a1 = labirintoCompleto.getStanzaCorrente().getStanzaAdiacente("nord").getAttrezzo("chiave");
		Attrezzo a2 = labirintoCompleto.getStanzaCorrente().getStanzaAdiacente("nord").getAttrezzo("lanterna");
